...
export default async function BeneficiariosAdminPage({
  searchParams,
}: {
  searchParams: Promise<{ q?: string; status?: string }>
}) {
  const { q: searchRaw, status: statusFilter } = await searchParams
  ...
  return (
    <div className="space-y-6">
      ...
      <form method="get" action="/admin/beneficiarios" className="flex flex-wrap gap-2">
        <input
          name="q"
          defaultValue={search}
        ...
        {hasFilters && (
          <Button type="button" variant="ghost" asChild>
            <Link href="/admin/beneficiarios">Limpar filtros</Link>
          </Button>
        )}
      </form>

      <AdminFilterBar filters={filters} basePath="/admin/beneficiarios" />
      ...
      <Card className="overflow-hidden p-0">
        <div className="table-scroll">
          <table className="data-table">
            <thead>
              <tr>
                {['Nome', 'E-mail', 'Telefone', 'Inscrições', 'Status', 'Cadastrado em', 'Ações'].map(h => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {!beneficiaries || beneficiaries.length === 0 ? (
                ...
                beneficiaries.map(b => (
                  <tr key={b.id}>
                    ...
                    <td>{b.phone ?? '—'}</td>
                    <td>
                      <AdminNavLink href={`/admin/inscricoes?beneficiary_id=${b.id}&view=person`}>
                    ...
                    <td>
                      <div className="flex flex-wrap gap-1">
                        ...
                        <Link href={`/admin/inscricoes?beneficiary_id=${b.id}&view=person`}>
