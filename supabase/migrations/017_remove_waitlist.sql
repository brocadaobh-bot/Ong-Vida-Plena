-- Remove lista de espera: migra inscrições existentes para pendente e ajusta contagem de vagas.
...
BEGIN
  ...
  FROM enrollments
  WHERE class_id = p_class_id AND status IN ('confirmed', 'pending');
...
BEGIN
  ...
  SELECT
    ...
    GREATEST(
      0,
      c.capacity - (COUNT(*) FILTER (WHERE e.status IN ('confirmed', 'pending')))::INTEGER
